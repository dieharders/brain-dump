        ...
        <div className="flex w-full flex-col items-stretch justify-items-stretch gap-4 pb-4">
          <div className="flex flex-col gap-2">
            ...
            <div className="w-full">
              <Select id="model_select" placeholder="Select model" name="Installed models" value={state.id || undefined} items={installedModels} onChange={(val) => setState((prevState) => ({ ...prevState, id: val }))} />
            </div>
            {/* Select a file (quant) to load for the model */}
            {state.id &&
              <div className="w-full">
                <Select id="file_select" placeholder="Select file" name="Available files" value={state.filename || undefined} items={installedFiles} onChange={(val) => setState((prevState) => ({ ...prevState, filename: val }))} />
              </div>}
          </div>
        </div>
      </div>
